...
These elements are **not supported through imports**, so they must be copied into the frontmatter of every agentic workflow. See `.github/actions/select-copilot-pat/README.md` for full documentation.
...
The action interface always uses `SECRET_0` through `SECRET_9` for its environment variables. Map those to whichever secret pool the workflow uses in this repo, such as `COPILOT_PAT_#` or `COPILOT_HELLO_#`, and keep the `engine.env.COPILOT_GITHUB_TOKEN` `case()` expression aligned with the same pool.
...
```yaml
  ...
  steps:
    ...
    - id: select-copilot-pat
      ...
      env:
        # Map these SECRET_# values to the secret pool used by the workflow.
...
- If no pool secrets are configured, the expression falls back to the default `COPILOT_GITHUB_TOKEN` secret.
- Do **not** specify `engine: copilot` as a simple string — use the object form shown above so the `env:` override can be included.
- If the workflow uses a pool name other than `COPILOT_PAT_#`, update both the `SECRET_#` mappings and the `case()` expression to match that pool.
